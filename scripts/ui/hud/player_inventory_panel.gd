...
@export var title: String = "Inventario"
...
func set_inventory(inventory: InventoryContainer) -> void:
...
func refresh() -> void:
	...
	if _inventory == null:
		_status_label.text = "Inventario: nao configurado"
		_items_label.text = ""
		return

	var items := _inventory.get_all_resources()
	var item_lines := _format_items(items)
	_status_label.text = "Itens do jogador"
