...
func _ready() -> void:
	...
	print("[NPC] Cena de teste Wave 4 pronta. Olhe para a Casa Civil e interaja para criar civis. Teclas debug: %s." % debug_status)
...
func _select_npc_by_index(index: int) -> void:
	...
	if index < 0 or index >= npcs.size():
		print("[NPC] Cena de teste: indice de NPC invalido %d." % index)
		return
...
func _select_npc(npc: NPCBase) -> void:
	if not _is_valid_npc(npc):
		print("[NPC] Cena de teste: tentativa de selecionar NPC invalido.")
		return
	...
	print("[NPC] Cena de teste: selecionado %s." % _selected_npc.npc_name)
...
func _deselect_current() -> void:
	...
	if _command_menu != null:
		_command_menu.close_menu()
	print("[NPC] Cena de teste: selecao limpa.")
...
func _order_follow_player() -> void:
	if not _has_selected_npc():
		return
	if _player == null:
		print("[NPC] Cena de teste: jogador nao encontrado para FOLLOW_PLAYER.")
		return
...
func _has_selected_npc() -> bool:
	...
	print("[NPC] Cena de teste: nenhum NPC selecionado.")
	return false
...
func _connect_command_menu() -> void:
	if _command_menu == null:
		print("[NPC] Cena de teste: menu contextual nao encontrado.")
		return
...
func _connect_creation_menu() -> void:
	if _creation_menu == null:
		print("[NPC] Cena de teste: menu de criacao nao encontrado.")
		return
...
func _on_house_npc_created(npc: NPCBase) -> void:
	...
	if npc != null and is_instance_valid(npc):
		print("[NPC] Cena de teste: civil criado conectado ao menu de ordens: %s." % npc.npc_name)
...
func _on_menu_patrol_requested(npc: NPCBase) -> void:
	if not _is_valid_npc(npc):
		return
	_selected_npc = npc
	# Patrol completo requer waypoints; por ora reutiliza destino frente como ponto unico.
	_order_move_forward()
